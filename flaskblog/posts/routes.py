...
@posts.route('/post/<int:post_id>')  # route dedicated to each post separately
def post(post_id):
    post = Post.query.get_or_404(post_id)
    return render_template('post.html', post=post, title=post.title)


@posts.route('/post/<int:post_id>/update', methods=['POST', 'GET'])  # route dedicated to updating a post
@login_required
